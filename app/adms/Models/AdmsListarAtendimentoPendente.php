...
class AdmsListarAtendimentoPendente
{
    ...
    public function listarAtendimento($PageId = null)
    {
        ...
        $listarAtendimento->fullRead("SELECT aten.id, aten.descricao, aten.created, aten.inicio_atendimento, aten.at_tempo_restante, aten.at_iniciado, aten.at_tempo_excedido, 
                        ...
                        ORDER BY created DESC LIMIT :limit OFFSET :offset", "usuario=".$_SESSION['usuario_id']."&adms_sits_atendimento_id=4&prioridade=1&adms_s_atend_func_id=4&limit={$this->LimiteResultado}&offset={$offset}");
    ...
    public function listarAtendimentoUrgente()
    {
        ...
        $listarAtendimentoUrgente->fullRead("SELECT aten.id, aten.descricao, aten.created, aten.inicio_atendimento, aten.at_tempo_restante, aten.at_iniciado, aten.at_tempo_excedido, 
                        demanda.nome demanda, 
                        emp.nome nome_empresa, emp.fantasia fantasia_empresa, 
                        situacao.nome nome_situacao, situacao.id id_situacao, 
                        cr.cor,
    ...
    public function listarAtendimentoFinalizado()
    {

        $listarAtendimentoF = new \App\adms\Models\helper\AdmsRead();
        $listarAtendimentoF->fullRead("SELECT aten.id, aten.descricao, aten.created, aten.inicio_atendimento, aten.at_tempo_restante, aten.at_iniciado, 
                        aten.at_tempo_excedido, aten.fim_atendimento,  
                        demanda.nome demanda, 
                        emp.nome nome_empresa, emp.fantasia fantasia_empresa, 
                        situacao.nome nome_situacao, situacao.id id_situacao, 
                        cr.cor,
                        ...
                        INNER JOIN adms_sits_atendimentos_funcionario sitAtenFun ON sitAtenFun.id=aten.adms_sits_atendimentos_funcionario_id 
                        INNER JOIN adms_cors cor_sitAtenFun ON cor_sitAtenFun.id=sitAtenFun.adms_cor_id 
                        WHERE aten.adms_funcionario_id =:usuario AND aten.adms_sits_atendimento_id <>:adms_sits_atendimento_id  
                        AND aten.adms_sits_atendimentos_funcionario_id =:adms_s_atend_func_id 
                        ORDER BY created DESC", "usuario=".$_SESSION['usuario_id']."&adms_sits_atendimento_id=4&adms_s_atend_func_id=4");
        $this->Resultado = $listarAtendimentoF->getResultado();
        //var_dump($this->Resultado);
        return $this->Resultado;
    }
