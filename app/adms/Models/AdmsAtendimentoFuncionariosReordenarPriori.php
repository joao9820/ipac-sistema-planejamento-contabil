...
class AdmsAtendimentoFuncionariosReordenarPriori {
    ...
    private $duracaoAtv;
     ...
     function getResultado() {
        ...
        public function reordenarAtvPriori($ordem) {  //Os valores necessários podem vir de outros métodos
        ...
        $updateOrdem->exeUpdate("adms_atendimento_funcionarios", $this->ordem, "WHERE ", $ParseString);
        ...
        foreach ($resultadoBD as $novaOrdem) {
            ...
            $reordemDia->defineData($this->FuncId, $this->dataOrdemPriori, $reordemHoraInicio); //30/04
            ...
            if ($this->novaData != $this->dataOrdemPriori) { //Se sim significa que a data de inicio mudou e consequentemente a hora também, o tempo excedeu
                ...
                $this->dataOrdemPriori = $this->novaData; //Pois a data de compração fica sendo esta e só entrará nessa condição novamente se o dia mudar
                ...
                $this->DadosOrd['data_inicio_planejado'] = $this->dataOrdemPriori;
    ...
    public function buscarOrdem($func_i = NULL) { //Busca a ordem que está sendo apagada ou em relação ao id, atualizada
        ...
        $this->dataOrdemPriori = $ordem[0]['data_inicio_planejado'];
        //die();
    }
